...
Value king_safety_adjustment(const Position& pos, int setting) {
    if (setting == 100)
        return VALUE_ZERO;
    auto pressure = [&](Color color) {
        ...
        int danger = 0;
        while (zone)
            danger += popcount(pos.attackers_to(pop_lsb(zone)) & pos.pieces(~color));
        return danger;
    };
    return Value(((pressure(BLACK) - pressure(WHITE)) * 4 * (setting - 100)) / 100);
}
...
void Search::Worker::start_searching() {
    ...
    const bool mctsEnabled = options.count("MCTS Enabled") && bool(options["MCTS Enabled"]);
    const bool strategyMcts = options.count("Search Strategy")
                           && (options["Search Strategy"] == "MCTS"
                               || options["Search Strategy"] == "Montecarlo");
    bool uciPvSent = false;
    if (mctsEnabled || strategyMcts)
        run_monte_carlo();
    else
    {
    ...
    Experience::on_search_complete(bestThread->rootPos, bestThread->rootMoves,
                                   bestThread->rootMoves[0].score, evalScore,
                                   bestThread->rootDepth, bestThread->limits,
...
void Search::Worker::run_monte_carlo() {
    ...
    const int configured = options.count("MCTS Rollout Depth") ? int(options["MCTS Rollout Depth"]) : 12;
    const int depthHint = limits.depth ? std::clamp(2 * limits.depth, 4, MAX_PLY - 1)
                                       : std::clamp(configured, 4, MAX_PLY - 1);
    const auto result = MCTS::analyze(rootPos, rootPos.side_to_move(), options, limits,
                                      stopRequested, depthHint);
    nodes.store(result.nodesVisited ? result.nodesVisited : result.simulations,
                std::memory_order_relaxed);
    tbHits.store(0, std::memory_order_relaxed);
    rootDepth = Depth(depthHint);
    selDepth = depthHint;
    auto toValue = [](double pawns) {
        return Value(std::clamp(int(std::round(pawns * PawnValue)), -VALUE_MATE, VALUE_MATE));
    };
    for (auto& rm : rootMoves)
    {
        const Move move = rm.pv[0];
        const auto it = std::find_if(result.moveStats.begin(), result.moveStats.end(),
          [&](const MCTS::MoveStats& s) { return s.move == move; });
        const Value score = it == result.moveStats.end() ? -VALUE_INFINITE
                                                         : toValue(it->averageValue);
        rm.score = rm.previousScore = rm.averageScore = rm.uciScore = score;
        rm.meanSquaredScore = score == -VALUE_INFINITE ? score : Value(int(score) * int(score));
        rm.selDepth = depthHint;
        rm.effort = it == result.moveStats.end() ? 0 : it->visits;
    }
    std::stable_sort(rootMoves.begin(), rootMoves.end());
    if (!result.principalVariation.empty())
    {
        auto it = std::find(rootMoves.begin(), rootMoves.end(), result.principalVariation[0]);
        if (it != rootMoves.end())
        {
            it->pv.clear();
            for (Move move : result.principalVariation)
                if (move.is_ok()) it->pv.push_back(move);
        }
        std::stable_sort(rootMoves.begin(), rootMoves.end());
    }
    if (rootMoves[0].score == -VALUE_INFINITE)
        rootMoves[0].score = rootMoves[0].averageScore = rootMoves[0].uciScore = VALUE_ZERO;
}
...
Value Search::Worker::evaluate(const Position& pos) {
    Value value = Eval::evaluate(network[numaAccessToken], pos, accumulatorStack, refreshTable,
                                 optimism[pos.side_to_move()]);
    value += pos.side_to_move() == WHITE ? contemptValue : -contemptValue;
    const Value safety = king_safety_adjustment(pos, kingSafetySetting);
    value += pos.side_to_move() == WHITE ? safety : -safety;
    return value;
}
