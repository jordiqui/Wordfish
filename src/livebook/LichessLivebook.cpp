...
std::vector<std::pair<std::string, Analysis>> LichessLivebook::lookup(const Position& position) {
    ...
    {
        std::cerr << "Error parsing JSON: invalid payload" << std::endl << readBuffer << std::endl;
        return ret;
    }

    if (!data.contains("moves") || !data["moves"].is_array())
    {
        std::cerr << "Error parsing JSON: \"moves\" not found" << std::endl << readBuffer << std::endl;
        return ret;
    }
    ...
    for (auto move : moves)
    {
        ...
        {
            std::cerr << "Error parsing JSON: \"uci\" not found" << std::endl << readBuffer << std::endl;
            continue;
        }
